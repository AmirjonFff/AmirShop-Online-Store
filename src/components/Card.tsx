    ...
    return (
        <Card h={'100%'} shadow="sm" className='w-full cursor-pointer myCard border-none' padding="lg" radius="md" withBorder>
            ...
            <Group onClick={() => navigate('/device/' + card.id)} justify="space-between" mt="md" mb="xs">
                <Text className='text-[13px] hidden md:block sm:text-[16px]' fw={500}>{card.title.length < 28 ? card.title : `${card.title.slice(0, 40)}...`}</Text>
                <Text className='text-[13px] md:hidden sm:text-[16px]' fw={500}>{card.title.length < 10 ? card.title : `${card.title.slice(0, 10)}..`}</Text>
            </Group>
